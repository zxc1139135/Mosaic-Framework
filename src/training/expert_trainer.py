...
Implements domain specialization training with knowledge distillation.
"""
...
class ExpertTrainingConfig:
 ...
 # GPU
 use_single_gpu: bool = False # True: GPU, False: GPU
 single_gpu_id: int = 0 # GPUGPU ID
...
class DistillationLoss(nn.Module):
 ...
 : L_target,k = KL(P_T || P_k) + λ1|μ_T - μ_k|² + λ2|σ²_T - σ²_k|²

 Args:
 student_logits: logits [batch, seq, vocab]
 teacher_logits: logits [batch, seq, vocab]
 student_hidden: 
 teacher_hidden: 
...
class DiversityLoss(nn.Module):
 ...
 : L_diversity,k = -E[Σ_{j≠k} log D(P_k, j)]

 ： JS 

 Args:
 expert_logits: logits [batch, seq, vocab]
 other_expert_logits: logits
...
class CrossExpertDistillationLoss(nn.Module):
 ...
 : L_cross,k = Σ_{j∈N_k} w_{k←j} * KL(P_j || P_k)

 Args:
 student_logits: logits [batch, seq, vocab]
 neighbor_logits: logits
 neighbor_weights: 
...
class ExpertTrainer:
 ...
 device_id: Optional[int] = None, # ：ID
 ):
 """
 Args:
 expert_model: 
 ...
 : L_k = α·L_domain + β·L_target + γ·L_cross + δ·L_diversity
...
class MultiExpertTrainer:
 ...
 device_ids = [None] * self.num_experts # CPU
 elif config.use_single_gpu:
 # GPU：GPU
 gpu_id = min(config.single_gpu_id, num_gpus - 1) # GPU ID
